...
func New(
	...
	// судя по поведению, сообщение в топик приходило уже после запуска приложения,
	// поэтому пришлось добавить этот код:
	// по сути это ожидание, сообщения из kafka (не более 1 секунды) с первым курсом валют
	// до того как поднять http сервер (т.к. он поднимается на lifecycle)
